...
export const getUserPost = async (req,res) => {
    try {
        const authorId = req.id;
        const posts = await Post.find({author:authorId}).sort({created:-1}).populate({
            path: 'author',
            select:'username, profilePicture'
        }).populate({
            path: 'comments',
...
export const getCommentsOfPost = async(req,res) => {
    try {
        ...
        const comments = await Comment.find({post: postId}).populate('auhor','username, profilePicture');
...
export const bookmarkPost = async (req,res) => {
    try {
        ...
        if(user?.bookmarks.includes(post._id)) {
            ...
            await user.save();
            return res.status(200).json({type:'unsaved', message: 'Post removed',
                success: true
            })
        }else {
